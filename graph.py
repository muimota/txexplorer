...
def getStepData(inputs, valueThreshold = 0):
    ...
    for txId in tqdm(txIds):
        ...
        if(len(partialstepdata['inputs']) < 10):
...
def exploreTransaction(txId,stepCount = 50,valueThreshold = 0):
    ...
    for step in range(startStep,stepCount):
        ...
        #free previous step
        if len(data['stepdata']) > 2:
            del(data['stepdata'][-2]['inputs'])
        del(inputs)
        gc.collect()
        ...
        if unsavedInputsCount > 100 or len(inputs) == 0 or step == stepCount - 1:
